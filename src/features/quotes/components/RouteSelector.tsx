...
// Your SVG dimensions (from world-map-dark.svg)
const MAP_WIDTH = 784.077;
const MAP_HEIGHT = 458.627;

// World map asset (already in your project)
const WORLD_MAP_URL = "/world-map-dark.svg";

// Port positions as real-world lat/lon (approx)
type PortGeo = { lat: number; lon: number };

const PORT_GEO: Record<string, PortGeo> = {
  // AFRICA / EUR
  "CASABLANCA (MAP)": { lat: 8.0,  lon: -16.0 },   // your working value
...
// Equirectangular projection from lat/lon into SVG coordinates
...
function inferHub(pol: string, pod: string, mode: TransportMode): string | null {
  ...
  const isEUorNA = (p: string) =>
    /(CASABLANCA|TANGER|AGADIR|ROTTERDAM|HAMBURG|VALENCIA|MARSEILLE|NEW YORK|LOS ANGELES)/.test(
      p
    );
...
function computeEtaFromTransit(
...
const WorldMap = ({
  ...
  const routeColor =
    mode === "AIR" ? "#fbbf24" : mode === "ROAD" ? "#22c55e" : "#3b82f6";

  const distanceKm = Math.round(dist * 0.5); // scale so numbers aren't crazy
  ...
  const etaIso =
    estimatedArrivalDate ||
    computeEtaFromTransit(requestedDepartureDate, transitTime);
  const etaShort = formatShortDate(etaIso);

  const effectiveTransit =
    transitTime ||
    (etaIso && requestedDepartureDate
      ? Math.round(
          (new Date(etaIso).getTime() -
            new Date(requestedDepartureDate).getTime()) /
            (1000 * 60 * 60 * 24)
        )
      : null);

  return (
    <div className="relative w-full h-64 rounded-xl border border-slate-800 bg-slate-950 overflow-hidden shadow-[0_16px_50px_rgba(15,23,42,0.9)]">
      {/* Background depth */}
      ...
      >
        ...
        <image
          ...
          height={MAP_HEIGHT}
          preserveAspectRatio="xMidYMid slice"
          xlinkHref={WORLD_MAP_URL}
        />

        {/* Dark overlay so route/UI stand out */}
        <rect
          x="0"
          y="0"
          width={MAP_WIDTH}
          height={MAP_HEIGHT}
          fill="url(#darkenOverlay)"
        />
      ...
      {/* TOP HUD: route + mode + distance + dates + hub */}
      <div className="pointer-events-none absolute inset-x-4 top-3 flex items-center justify-between text-[11px] font-medium text-slate-300">
        <div className="flex items-center gap-2">
          <span className="rounded-full border border-slate-700/70 bg-slate-900/80 px-2 py-0.5">
            {pol || "SET POL"}
          </span>
          ...
          {hub && (
            <span className="ml-2 rounded-full border border-slate-700/70 bg-slate-900/80 px-2 py-0.5 text-[10px] text-sky-300">
              Via {hub}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          {etdShort && (
            <span className="rounded-full border border-slate-800 bg-slate-900/80 px-2 py-0.5 text-[10px] text-slate-300">
              ETD {etdShort}
              {etaShort ? ` · ETA ${etaShort}` : ""}
            </span>
          )}
          <span className="rounded-full border border-slate-700/70 bg-slate-900/90 px-2 py-0.5 uppercase tracking-[0.08em]">
            {mode.replace("_", " ")}
          </span>
          <span className="rounded-full border border-slate-800 bg-slate-900/90 px-2 py-0.5 font-mono text-[10px]">
            ~{distanceKm.toLocaleString()} km
          </span>
        </div>
      </div>

      {/* BOTTOM LEGEND + LEG BREAKDOWN */}
      <div className="pointer-events-none absolute inset-x-4 bottom-3 flex items-center justify-between text-[10px] text-slate-400">
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1">
            <span className="h-1 w-4 rounded-full bg-slate-400/80" /> Grid
          </span>
          <span className="flex items-center gap-1">
            <span className="h-1 w-4 rounded-full bg-sky-400/90" /> Route
          </span>
          <span className="flex items-center gap-1">
            <span className="h-2 w-2 rounded-full bg-emerald-400" /> Destination
          </span>
        </div>

        <div className="flex items-center gap-2">
          {effectiveTransit && (
            <span className="rounded-full border border-slate-800 bg-slate-900/90 px-2 py-0.5 font-mono">
              {effectiveTransit} d
            </span>
          )}
          {hub && pol && pod && (
            <span className="hidden sm:flex items-center gap-2 text-slate-300">
              <span className="rounded-full bg-slate-900/90 px-2 py-0.5 border border-slate-800">
                Leg 1 · {pol.split(" (")[0]} → {hub}
              </span>
              <span className="rounded-full bg-slate-900/90 px-2 py-0.5 border border-slate-800">
                Leg 2 · {hub} → {pod.split(" (")[0]}
              </span>
            </span>
          )}
        </div>
      ...
      <div className="grid gap-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <div className="group relative">
              <Input
                list="ports"
                className="h-9 border-slate-200 bg-white pl-8 text-sm font-semibold uppercase text-slate-700 transition-all focus:border-blue-500"
                value={pol}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={incoterm}
              onValueChange={(val) => setIncoterm(val as Incoterm)}
            >
              <SelectTrigger className="h-9 border-slate-200 bg-slate-50 font-medium">
                <SelectValue />
              </SelectTrigger>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setIdentity("requestedDepartureDate", e.target.value)
                }
              />
            </div>
            <div className="border-l border-slate-200 pl-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setIdentity("estimatedArrivalDate", e.target.value)
                }
              />
            </div>
          </div>
        ...
        <div className="flex items-center justify-between border-t border-slate-100 pt-2">
          ...
          <div className="flex items-center gap-2">
            <Input
              type="number"
              className="h-7 w-16 bg-slate-50 text-right text-xs font-mono"
              value={transitTime}
              onChange={(e) =>
                setIdentity("transitTime", parseInt(e.target.value) || 0)
              }
            />
