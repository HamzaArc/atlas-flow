...
export function QuoteHeader({ onBack }: QuoteHeaderProps) {
  ...
  // Updated ReadOnly/Locked logic to include CONVERTED status
  const isReadOnly = status === 'ACCEPTED' || status === 'REJECTED' || status === 'SENT' || status === 'CONVERTED';
  const isLocked = status === 'ACCEPTED' || status === 'REJECTED' || status === 'CONVERTED';
  ...
  const handleConvertToJob = async () => {
      // 1. Fetch full quote data to ensure we have the latest options/cargo from DB/Service
      const fullQuote = await QuoteService.getById(id);
      
      if (!fullQuote) {
          // FIX: Changed "destructive" to "error" to match your toast type definition
          toast("Error: Could not retrieve full quote details.", "error");
          return;
      }

      // 2. Initialize the Dossier
      initializeFromQuote(fullQuote);

      // 3. Update Status to avoid duplicates
      await setStatus('CONVERTED');

      // 4. Navigate to the new dossier
      // The ID generated by initializeFromQuote is `new-quote-${quote.id}`
      navigate(`/dossiers/new-quote-${id}`);
      
      toast("Shipment Created: Quote converted successfully.", "success");
  };
  ...
  const StatusStepper = () => (
      <div className="flex items-center mr-6 bg-slate-50 rounded-lg p-1 border border-slate-200">
          {STEPS.map((step, idx) => {
              // Map CONVERTED to ACCEPTED for visual stepper
              const effectiveStatus = status === 'CONVERTED' ? 'ACCEPTED' : status;
              
              const isActive = step.id === effectiveStatus;
              const currentIdx = STEPS.findIndex(s => s.id === effectiveStatus);
      ...
      <div className="flex items-center justify-between px-6 h-16 bg-white">
        ...
        <div className="flex items-center">
           ...
           <div className="flex gap-2 items-center">
               ...
               {/* NEW: Convert to Job Button (Restored) */}
               {(status === 'ACCEPTED' || status === 'CONVERTED') && (
                   ...
                   >
                       ...
                       {status === 'CONVERTED' ? "Job Created" : "Convert to Job"}
                   </Button>
               )}
           </div>
        </div>
